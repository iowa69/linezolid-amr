...
- **Real-world cohort.** A clinical validation across Gram-positive isolates (*Staphylococcus aureus*, *Enterococcus faecalis*, *Enterococcus faecium*, *Streptococcus pneumoniae*) is described in the manuscript in preparation. The per-isolate outputs are not yet in this repository, so those results are not restated here as a property of the software — see [Manuscript](#manuscript).
